...
function initPendingQueue() {
  ...
  pendingUnsubscribe = onSnapshot(q, async (snapshot) => {
    ...
    // Sort descending by createdAt in memory (robust against missing/pending composite indexes)
...
function initApprovedVisitsLog() {
  ...
  approvedUnsubscribe = onSnapshot(q, async (snapshot) => {
    ...
    const docs = [...snapshot.docs]
      .sort((a, b) => {
        const aTime = a.data().approvedAt?.toMillis ? a.data().approvedAt.toMillis() : 0;
        const bTime = b.data().approvedAt?.toMillis ? b.data().approvedAt.toMillis() : 0;
        return bTime - aTime;
      })
      .slice(0, 25);
